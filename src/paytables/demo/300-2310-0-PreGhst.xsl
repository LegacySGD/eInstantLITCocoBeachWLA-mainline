			...
			<lxslt:component prefix="my-ext" functions="formatJson,retrievePrizeTable,getType">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNamesDesc)
					{
						...
						// Main Game Headers //
						///////////////////////
						const headerTranslations  = ['coconut'];
						...
						for (var headerIndex = 0; headerIndex < scenarioMainGame.length; headerIndex++)
						{
							r.push('<td>');
							canvasIdStr   = 'cvsTitleMain' + 'Header' + headerIndex.toString();
							elementStr    = 'eleTitleMain' + 'Header' + headerIndex.toString();
							showHeader(canvasIdStr, elementStr, cellWidth, getTranslationByName(headerTranslations[0], translations) + " " + (headerIndex +1).toString());
							r.push('</td>');
						}
						r.push('</tr>');

						///////////////////////////////////////////////
						// Play Game to see if there are any winners //
						///////////////////////////////////////////////
						var winningLetters = [];
						var dataCell = "";
						var playBonus = false;
						var prizeCounts = [0,0,0,0,0,0,0,0];
						var bonusCounts = [0,0,0,0,0,0,0,0];
						for (var symbIndex = 0; symbIndex < scenarioMainGame.length; symbIndex++)
						{
							dataCell = scenarioMainGame[symbIndex];
							if ((dataCell.length > 1) && (dataCell[1] == ':'))
							{
								playBonus = true;
								bonusCounts[dataCell[2].charCodeAt(0)-65]++;
							}
							else
							{
								prizeCounts[dataCell[0].charCodeAt(0)-65]++;
								if (prizeCounts[dataCell[0].charCodeAt(0)-65] == 3)
								{
									winningLetters.push(dataCell[0]);
								}
							}
						}
						...
						for (var symbIndex = 0; symbIndex < scenarioMainGame.length; symbIndex++)
						{
							...
							if ((dataCell.length > 1) && (dataCell[1] == ':'))
							{
								symbCell  = getTranslationByName('bonusGame', translations);
								isBonusTrigger = true;
							}
							else
							{
								symbCell  = convertedPrizeValues[getPrizeNameIndex(prizeNames, dataCell[0])];
								isBonusTrigger = false;
							}
							canvasIdStr   = 'cvsTitleMain' + symbIndex.toString();
							elementStr    = 'eleTitleMain' + symbIndex.toString();
							boxColourStr  = (isBonusTrigger) ? colourRed : (winningLetters.indexOf(dataCell[0]) != -1) ? colourLime : colourWhite;
						...
						// Check for 3 of a symbol
						for (var i = 0; i < prizeTypes; i++)
						{
							if (prizeCounts[i] == 3)
							{
								mainWinTotal += getPrizeInCents(convertedPrizeValues[i]);
							}
						}
						if (mainWinTotal > 0)
						{
							r.push('<p>' + getTranslationByName('wins', translations) + " " + getCentsInCurr(mainWinTotal) + '</p>');
						}

						if (playBonus)
						{
							r.push('<p><br>' + getTranslationByName("bonusGame", translations) + '</p>');
							for (var i = 0; i < prizeTypes; i++)
							{
								if (bonusCounts[i] == 1)
								{
									bonusWinTotal += getPrizeInCents(convertedPrizeValues[i]);
								}
							}
							if (bonusWinTotal > 0)
							{
								r.push('<p>' + getTranslationByName('wins', translations) + " " + getCentsInCurr(bonusWinTotal) + '</p>');
							}
						}
						r.push('<p></p>');
					...
					function getMainGameData(scenario)
					{
						...
						{
							result.push(outcomeData[i].replace(/\s/g, ''));
						}
						return result;
					}
